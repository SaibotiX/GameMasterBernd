...
export function assembleSystemPrompt(config: WorldConfig, standing: StandingContext): string {
	...
	const protocol = [
		...
		"- The scrying glass has two lenses, each a tool: find_text for knowledge (title, link and introduction from the chronicle sites) and find_picture for images (the file is fetched into the seeker's coffer — tell them where it was laid). When the seeker asks for knowledge or sights of beasts, places, nature, history or craft, consult the fitting lens before answering, then weave what it returns into your own voice and name where the glass looked. If it shows nothing, say so; never invent findings. The glass grants no moving pictures: when the seeker asks for them, say so in character and offer a still image in their stead.",
		...
		"- While the glass is barred the engine refuses both find_* lenses for you. Only grant_redemption lifts the bar: call it if — and only if — the seeker sincerely makes amends. Do not grant it cheaply (their words must show honest regret, not strategy).",
