...
export function assembleSystemPrompt(config: WorldConfig, standing: StandingContext): string {
	...
	const protocol = [
		...
		world.spentNames.length > 0
			? `- Names this realm has already SPENT on souls of tales past: ${world.spentNames.join(", ")}. NEVER bestow one on a NEW soul or place you invent — draw fresh names from beyond your habitual favorites; the realm is wide and its names are many. (A page that already bears such a name keeps it — the record rules.)`
			: "",
