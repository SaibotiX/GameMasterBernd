...
| House lane & ledger | ● | R12 · R29 · [02](02-friends-web-service.md) item 8 · `deploy/host/gateway/` | LIVE, round closed 2026-08-09 (drain test + the played turn green); open: telemetry tunes the grant/estimate numbers · the side-call spend question — gmchat's side calls (GM table, chronicler, fate planner, judges) meter through the gateway since the 2026-08-09 fix but never land in pi's cost stamps (out-of-session by design), so reconcile's 5% drift tolerance can page falsely on chat-heavy sittings; proposal: side calls send a marker header, the ledger tags those rows, reconcile subtracts them — maintainer's ruling wanted · the fix's lane eyeball (/gm + /history long) rides the words round · Gemini EEA/under-18 facts re-verified 2026-08-08 |
