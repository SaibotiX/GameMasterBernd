...
| Ops teaching layer (the deploy guide) | ◐ | `deploy/guide/` — explanations · how-tos · reference · blueprint; live ops truth stays `deploy/README.md` (the runbook wins on conflict) | landed 2026-08-17, pages synced at `4ba34eb` · open, both the next round's threads: the freshness workflow (`/guide-sync` + its own R entry — re-check pages when deploy/ moves) · the external minimal template at `~/Desktop/CurrPC/Programming/Example_Web_Server_Project` (built from guide 30's blueprint, verified by running it) |
