...
**The 2026-08-06 business/naming sitting** answered the commercial unknowns in six evidence sections of [06](06-research-log.md) — hosting · domain/TLD · trademark · Austrian founding & tax · payments · compliance gaps — and made the coverage register law ([09-coverage.md](09-coverage.md), R15: zero silent unknowns). The proposals now await **rulings, collected in the register's ◐/○ rows**: the name (clearance clean; worldconsole.com squatted at $3,595, every other TLD free) · the TLD basket (.games + .at + .eu ≈ €50/yr at INWX proposed) · the box (netcup Vienna ≈ €6/mo proposed, cross-provider backups) · the Impressum address strategy · the age policy (18+ proposed) · the stage-2 payments branch (Stripe direct vs Stripe-MoR) · the maintainer's founding branch (employment/age — input only they can give). Nothing was adopted against standing law; the one would-be deviation (a non-Stripe merchant of record) is flagged inside the payments section and arises only if that branch is ever chosen.

Next, in order:

1. **Stage 1 build** — the [02](02-friends-web-service.md) checklist: **the 2026-08-06 rulings first** (name · TLD · box · address · age — the register's open edges, a short maintainer pass), then fallback rung first (Dockerfile → in-container smoke → Caddy + secret links, with disclosure and the LICENSE line from day one), then vault · house lane · shipper · panes ([08](08-stage1-web-ui.md)). First artifact: the Dockerfile.
