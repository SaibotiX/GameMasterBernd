...
| `LICENSES/` + the all-rights notice | `tools/yt-dlp/` — shed with the retired video lens (R23); back in only if `find_video` ever revives |
...
1. **Private GitHub repo, invited collaborators** — revocable per person, updates via `git pull`, visible who has access. (`--recurse-submodules` only matters if the retired video lens is ever revived — the yt-dlp submodule is dormant, and an empty checkout is fine.) Best for technical friends.
...
world-console-<os>/
  ...
  LICENSES/           ← pi MIT text (required when redistributing)
```
...
- python3/ffmpeg concerns shed with the retired video lens (R23) — if `find_video` ever revives, its old caveats (python3 for `python3 -m yt_dlp`, absent on Windows; ffmpeg licensing if ever distributed — see [04-licensing-and-ip.md](04-licensing-and-ip.md)) revive with it.
