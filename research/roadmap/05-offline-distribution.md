...
1. **Private GitHub repo, invited collaborators** — revocable per person, updates via `git pull`, visible who has access. Best for technical friends.
...
- Windows is untested territory for the game itself; expect SmartScreen warnings on unsigned binaries; one smoke test before any Windows friend gets a zip. (The old python3/ffmpeg caveats died with R24 — all three scrying lenses are plain HTTP, nothing beyond pi to install.)
