...
- `/history long` (the saga call) → costs credits, not rounds. (`find_video` left this list with R24: a Commons clip costs what a picture costs — the old line priced yt-dlp bandwidth that no longer exists; only genuinely expensive AI work is premium.)
...
| Premium credits | separate small currency for sagas and other genuinely expensive AI calls (video scrying left the list with R24) | Fables' credits pattern |
...
- **Media compliance:** resolved by R24 — every hosted tier runs **wiki-media only across all three lenses**: video comes from Commons (free-licensed by policy, per-file machine-readable license, the BY/BY-SA credit carried with every catch by the engine; mistagged strays exist on Commons until deleted — an accepted residual, and the source page rides each result). The YouTube-scraping exposure this bullet used to manage no longer exists. No user uploads exist anywhere in the product, so CSAM-reporting and DMCA-agent duties do not attach — true only while nothing player-uploaded is ever stored; any future upload feature re-opens that gate deliberately.
