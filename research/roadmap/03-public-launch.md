...
- **Business:** register per local rules; Stripe account; invoices/receipts. In the EU: VAT on digital services (OSS scheme), GDPR (privacy policy, data export/delete — our per-user volumes make deletion tractable by design).
...
- **Minors:** set the age policy (13+/16+) in the ToS before stage 3 opens. The constitution's content bounds are explicit as of 2026-08-04 — rule 4 binds both the scrying glass and the keeper's own telling — so the ToS and Steam's guardrail description reuse its wording ([07-steam-launch.md](07-steam-launch.md)).
