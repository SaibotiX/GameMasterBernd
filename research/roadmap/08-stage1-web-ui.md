...
- What the terminal cannot do is **show the chronicle and the scrying glass's catches**. The engine writes plain markdown under `data/world/<world>/<chronicle>/` (places, personas, quests, items, `ledger.md`, `record.md`, `chronicler.md`) and drops media into `data/downloads/` (`pic-*.jpg`, `clip-*.mp4`) — today `find_picture`/`find_video` only announce the saved file path as text. In local play the player opens these by hand; the web client renders them in place. **The file panes are not a debug view — they are the half of the game the terminal has always pointed at.**
