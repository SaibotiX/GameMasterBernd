...
- What the terminal cannot do is **show the chronicle and the scrying glass's catches**. The engine writes plain markdown under `data/world/<world>/<chronicle>/` (places, personas, quests, items, `ledger.md`, `record.md`, `chronicler.md`) and drops media into `data/downloads/` (`pic-*.jpg`; `clip-*.mp4` rest there from the retired video lens) — today `find_picture` only announces the saved file path as text. In local play the player opens these by hand; the web client renders them in place. **The file panes are not a debug view — they are the half of the game the terminal has always pointed at.**
...
The panes expose the **game folder only**: `config/` (constitution, worlds, laws, moods — readable per A5) and `data/` (the chronicle folders and `downloads/`). Read-only in v1: the terminal is how the player acts on the world; the panes are how they read it. Two exclusions, enforced in the file API, not the client: `config/youtube-cookies.txt` (player-supplied credential material) and anything outside the game folder — `~/.pi/agent/` (sessions, `auth.json`) is simply never mounted into the file server's root.
