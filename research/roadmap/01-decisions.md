...
**Decided:** The game is **World Console**, confirmed against the 2026-08-06 clearance (zero identical marks worldwide on TMview; five collision sweeps clean; the one ⚠ watch item — the Italian class-41 figurative whose device text contains "…THE GAME LAND WORLD CONSOLE" — gets its professional read before any EUTM filing, per the standing trademark plan). The studio name is **Hausregel** — the cleanest candidate on clearance (0 register hits worldwide, no brand collisions), needed as brand only: an Einzelunternehmer starts with no registered fantasy name (founding & tax section; R21), so holding the name costs nothing. With the name fixed, the trademark section's €0 step activates: dated public use of both names starts Austria's free protections (§ 9 UWG business designation, § 80 UrhG title) — **archive first-use evidence from the day the page is live.** Filings stay on the standing timetable — professional search, then the EUTM word mark in classes 41+9 with the SME-Fund voucher, at stage 2→3 — and that timetable remains a proposal, not yet a ruling. *(2026-08-18: the hub page is Hausregel's first public use — the H1 apex re-cut archives its first-use set per this step; R34.)* *(revised 2026-08-19, R36 — the names recut: the game becomes **GameMaster Bernd**, and "World Console" passes to the hub project (R35), whose page it names. The World Console clearance and first-use work stays on file, serving the name where it now stands; **Hausregel retires to the icebox**, clearance kept, revival condition named there. GameMaster Bernd starts with zero clearance work — its register-index sweep is due before the game's card goes public on the hub page; the professional-search-then-EUTM timetable applies to whichever names file.)*
*(2026-08-21 — the €0 step ran for World Console where it now stands: the hub's ruled page went live at the apex branding WORLD CONSOLE (WC repo H1b, commit `833fbd9`), and its first-use set was captured minutes after — `deploy/host/first-use/2026-08-21-world-console/` in the hub's box tree (rendered pages incl. the play-404 naming GameMaster Bernd, NOTE + SHA256SUMS), riding the hub's `borg/hub` nightly from the same day; receipts in the WC repo's `records/log.md` §2026-08-21 H1b. The game-era sets (2026-08-09, 2026-08-20-gamemaster-bernd) stand untouched beside it in this repo's box tree.)*
...
**Decided:** Stage 1 runs on a **netcup VPS 500 G12 in Vienna** (2 vCPU / 4 GB DDR5 / 128 GB NVMe, ≈ €5.96/mo incl. VAT) — the cheapest credible 4 GB box, session store on Austrian soil (R13's EU bound with margin), free click-AVV signed day 1, price locked a year into the 2026 wave once the term converts. Start on hourly billing; convert to the 12-month term when friends actually play. **The runner-up is part of the ruling: Hetzner CX23** — if netcup fails at signup or in service, that is the prepared exit (create and verify the Hetzner account before it is needed; the signup lottery is the documented risk there). The cross-provider lesson stands regardless of winner: nightly backups land at a *different* provider (Hetzner Storage Box as the borg target) plus the periodic pull to the maintainer's machine — an account lockout costs hours of uptime, never the research record. *(revised 2026-08-20 — the term stays hourly: the maintainer rules against converting for now. A box upsize may come first (more capacity/compute — hub load, real play), and a 12-month lock would sit crosswise to swapping the product; hourly keeps exit-any-hour with unused hours credited (the exits receipt, [06](06-research-log.md) §2026-08-17) at the €1.96/mo premium (€7.92 vs €5.96 incl. VAT). "When friends actually play" is superseded as the conversion trigger — the conversion re-opens once the box size settles, an upgrade landed or declined against real load. Named for that moment: whether netcup upsizes a term contract mid-term at all — verify then; the ⚠ netcup-AGB re-verify (hub log) still rides the conversion.)* *(2026-08-20, later the same sitting — the named question answered, evidence [06](06-research-log.md) §2026-08-20 upsize mechanics: a 12-month term would NOT block an upsize (AGB §3(7) — mid-term upgrade any time, in place, unused fees refunded) but each upgrade RESTARTS the 12-month lock, and downgrades/term-shortening are impossible — term is a one-way ratchet, exits only at term end (31-day notice). The genuinely unverified end is the hourly side itself: whether the hourly VPS 500 G12 gets CCP upgrade targets, and whether an in-place hourly→term conversion exists for this product — per-product; the CCP upgrade view or a ticket settles it. The stay-hourly ruling stands as taken; the CCP view is the named first step when an upsize turns concrete.)*
...
**Revisit when:** renewal (re-compare the field — ⚠ netcup/OVH price moves expected in the wave); or netcup's abuse desk ever locks the account (the runner-up activates). *(2026-08-20: + the box-size settle — the deferred term conversion re-opens there, per the revision above.)*
...
**Decided:** § 5 ECG is treated as applying from stage 1 (the cautious read, adopted), and the address on the Impressum/Offenlegung page is the **maintainer's private home address** — a real street-level, service-capable address satisfies both regimes in full (§ 5 ECG and the § 25 MedienG floor at once, no grey zone left); the maintainer enters it themselves when the page is built. The cost is privacy, accepted deliberately while the audience is invited friends behind secret links. **Standing gate, in the maintainer's own words: this is revised before the public release** — at stage-3 planning the address strategy comes back for a fresh ruling (the rented/virtual-office answer stays documented in the compliance section) before the page faces strangers. Stage 2 grows the block with the UID/GewO/WKO lines once the Gewerbe exists (the section's stage map). *(revised 2026-08-20 — the pick lands: the maintainer takes the rented address, §2026-08-18's Impressum-only tier at **impressumservice.at (RA Matutis, Hallein)**. Entitlement confirmed by the firm on the maintainer's own request; the block becomes **Tobias Maier · c/o RA Matutis · Fürstenstraße 5 · 5400 Hallein**, term to **2027-08-19**, €144 brutto/yr — full terms and duties in [06](06-research-log.md) §2026-08-20 (the inform-the-firm-per-publication duty · the firm opens all incoming mail · forwarding fees · no Google-Business use). The private street leaves the page at the `WC_IMPRESSUM_ADDRESS` swap on the box — the maintainer's own errand as ever, gated on the firm's activation gate (signed Vollmacht back + invoice paid) and seen-gated as a player-visible change. The proposal's honest limits ride the pick: the MedienG floor keeps the real Wohnort municipality (no street) on the page at medium confidence — the swap-time sub-question, the maintainer's call — and against strict § 5 ECG this is the defensible pre-commercial middle, not bulletproof. The stage-2 leg stands: a real Geschäftsadresse replaces this at the e.U./Gewerbe founding.)* *(2026-08-20, same day — executed: the maintainer declares the service ACTIVE (Vollmacht + payment complete) and the box-env swap ran the same sitting — the live Impressum renders `c/o RA Matutis, Fürstenstraße 5, 5400 Hallein` under the name line, three pages 200, receipt in the deploy runbook's 2026-08-20 state note. The swap-time sub-question is ruled: **the Wohnort-municipality line is omitted** — the maintainer's call; the medium-confidence floor reading stays on file as accepted residual risk. The firm's standing duties (inform-mail per publication · no confidential post · no Google-Business use · renewal re-verify 2027-08) are **tracked by the maintainer personally** — their instruction; sessions are released from chasing them, the 06 §2026-08-20 terms stay the reference.)*
...
**Revisit when:** stage-3 planning (the standing gate above) — or earlier if the page's audience widens for any reason. *(2026-08-18: the hub (R34) is exactly that widening, and the maintainer asked for the market — evidence landed in [06](06-research-log.md) §2026-08-18 "the rented address": the legal fit sharpened (the tatsächliche-Niederlassung prong; the § 25 Wohnort nuance; RSa/RSb mechanics) and the Austrian market priced (~€12/mo Impressum-only tier now, €45–75 netto Gewerbe-capable tier at founding). The pick — and with it this entry's revision — stays the maintainer's.)* *(2026-08-20: picked AND executed the same day — the revisions above. Remaining checkpoints: renewal 2027-08 (⚠ re-verify fee/terms; duties maintainer-tracked) · the founding replacement (stage 2) · the stage-3 gate unchanged.)*
...
## R28 — Standing law re-earns its place: the guardrail audit

*(added 2026-08-08, governance round, on the maintainer's instruction; method owned by `.claude/commands/audit-guardrails.md` (`/audit-guardrails`). Evidence: [06-research-log.md](06-research-log.md) §2026-08-08.)*

**Decided:** Guardrail freshness runs on two lanes. LANE ONE, on command: `/audit-guardrails` sweeps the standing law — both registries, CLAUDE.md's operational sections, the command files, the coverage register, the icebox — in a fixed order: integrity (dated headers, resolving supersession links, pointer documents agreeing with their targets) → every "Revisit when" clause EVALUATED against reality (has the condition actually fired?) → every ⚠ fact re-verified at its source (subagent research for external facts) → time backstop (entries untouched ~3 months with no fired trigger get FLAGGED, nothing more) → contradiction scan (entries against entries and against the documents that point at them — the one-truth check) → the icebox pass (R25's revive / stay / close review) → PROPOSALS ONLY: dated amendment drafts the maintainer rules one by one, and the sweep itself lands as a dated research-log entry recording the still-valid verdicts, so confirmation is never silent. LANE TWO, standing: every round wrap asks one question out loud — "did this round fire any standing Revisit-when trigger or contradict standing law?" — and a yes becomes a flagged item before the round closes. Full sweeps run at stage boundaries, after any salvage (R27), and whenever the maintainer smells drift. The sweep NEVER rewrites: agents draft, the maintainer rules (the audit-workflow law applied to the law itself); settled decisions do not reopen without a fired trigger or new evidence.

**Why:** A guardrail that outlives its truth is worse than none — it answers with yesterday, confidently. The research consensus matches the registries' own architecture: trigger-based review carries the real weight (the Revisit-when clauses are already standing law), calendar review alone decays into rubber-stamping, and the honest hybrid is triggers-primary with a cheap time backstop asking only whether a trigger fired unnoticed. The recorded-confirmation rule is the anti-theater tooth: "still valid" becomes evidence, not silence.
...
## R30 — Player mode: the console wears the world, not the workshop

*(added 2026-08-10, player-UI round, on the maintainer's ruling — ordering, enforcement depth and the allowlist cut settled in conversation; banner ownership delegated to the session. Evidence: the pi 0.84.1 surface survey and the registry sweep opening that round; the couplings it adds are rowed in [`pi-upgrades.md`](../design/pi-upgrades.md).)*

**Decided:** Friend containers carry `WC_PLAYER_UI=1` (baked into the image env) and the engine dresses the console for the player, hiding the workshop without deleting it. The footer keeps **only the game line** — wounds, level and mood stay public law (F5); pi's stats line and path line go dark, and the page strip's rounds note remains the player's one cost meter. Startup goes quiet (pi's own `quietStartup`, seeded by the entrypoint beside the trust and changelog seeds) under a **banner**: per-world art from `config/worlds/<id>.banner.txt` when the world brings its own, the World Console mark otherwise, closing with the world's title, the mood, and the first-command hint *(refined at the seen-gate, maintainer's proposition 2026-08-10: under the title stands the world's **face** — `worlds/<id>.intro.md`, a short descriptive summary of the world and its laws, never "you should"; the hint slims to "typing / lists every command" alone — mood already lives in the footer, and the board is theirs to find)*. The command surface narrows to **fifteen** — the game's twelve (`/web /pick /roll /ledger /quest /place /persons /record /thoughts /history /gm /dm`) plus pi's `/tree /new /resume`; `/compact` is ruled OFF the list (auto-compaction carries that weight unseen). Enforcement is two-layer: the autocomplete popup filters to the allowlist, and a gate at the editor's submit refuses the rest of pi's built-ins, `/limits`, pi's undocumented debug commands, and the `!`/`!!` bash escape — each with an in-register notice; an unknown `/word` still passes to the keeper, who deflects in character. Model and thinking keys and ctrl+z die at the same editor; **ctrl+o lives** (it is the game's own expand key for collapsed table talk) *(refined same round, build: **ctrl+t lives too** — it toggles thinking VISIBILITY, the live half of `/thoughts bernd` (pi applies the persisted setting only from the next sitting); only the thinking-LEVEL key shift+tab, a compute control, dies)*. The working row speaks in the keeper's voice; `PI_OFFLINE` keeps pi's update and changelog notices out of a friend's sitting. The maintainer's own play — no env var — keeps every surface exactly as before; `WC_PLAYER_UI=1 pi` previews the player's console anywhere. *(revised 2026-08-10, first-invite round, on the maintainer's ruling — "implement `/worlds` as command sixteen": the surface grows to **sixteen** — `/worlds` lists the console's worlds and binds a data-volume choice for the next `/new`; the stamp law untouched, a standing tale keeps its world. Shape and receipts: the build log's Phase 3 world-picker row.)* *(revised 2026-08-17, first-friends round, on the maintainer's ruling — the tester-notes gap becomes an in-play ask, not a seal-time one: the surface grows to **seventeen** — `/note <words>` sets a line down for the makers in the story folder's `notes.md`, shipped with the R13 batch and never read back into play; the player console invites one every 10 player turns, skippably (the default X=10 settles at the seen-gate). Shape and receipts: the build log's Phase 3.1 round record.)*

**Why:** The friend's first screen is unrepeatable, and what it showed was the workshop: token meters, dollar figures, a model id every frame — while the constitution forbids the keeper to "explain technology, models, or the real world behind the curtain" (config/constitution.md). Player mode fixes that inconsistency in standing law rather than fighting it. The curated subset already existed in embryo (the playtester guide's "you only need these"); the popup filter alone would be theater — pi executes a typed built-in before any extension hook, so only a submit-time gate actually enforces — and the same gate closes the `!` shell and `/share`'s gist door in one move.

**Rejected:** *Hiding the game footer line too* — wounds/level/mood are public meters by law (F5, mechanics §renown/wounds), the deploy probes assert the footer's mood mark, and an invisible state meter is the exact failure R14's strip rule guards against. *Forking or patching pi* — R1 stands; everything rides the documented extension API plus two shim-tier couplings, rowed and guarded. *Seeding a player `keybindings.json`* — the editor's action-handler filter kills the same keys without tmpfs seed choreography. *Hiding `/new`* — death law names it ("a new tale begins with /new"); it stays.

**Revisit when:** friend feedback asks for a hidden surface back (this entry is the terminal's re-cut trigger, as R14's is the page's) · stage 2 opens — strangers need onboarding surfaces friends don't · a pi upgrade changes the built-in command set (the unit drift test goes red; re-judge blocklist and allowlist) · R11's doors revive (`/limits` returns to the player surface — noted in its icebox entry).

## R31 — The teaching layer follows the machinery: the guide sync

*(added 2026-08-17, guide round thread B, on the maintainer's instruction; method owned by `.claude/commands/guide-sync.md` (`/guide-sync`); the guide's contract — stamps, additions, corrections — stays owned by `deploy/guide/README.md` §Staying fresh, which names this workflow as its rule 2. Evidence: the guide round itself (60ee021) — thirteen pages that quote live values by design, each stamped `Synced:` with the deploy/ state it was checked against.)*

**Decided:** The guide's freshness is a workflow, not a hope. `/guide-sync` re-checks `deploy/guide/` whenever `deploy/` moves: collect every page's `Synced:` hash (they may diverge — additions land with their own) → diff each to HEAD over `deploy/` excluding the guide (empty everywhere → "fresh", nothing touched) → map changed files to affected pages TWO ways, filename mention and topic ownership → re-read affected pages whole and EDIT DIRECTLY — the guide is a living document (contract rule 4); the audit lanes' proposals-only rule does not apply — with the machinery always winning: where guide and reality disagree, the guide moves, never the runbook or the files → stamps advance on every page checked, to the deploy-state commit it was checked against. New machinery with no home lands as a new page per the contract's addition rule; a deploy/ change that smells like a direction change is SURFACED, never silently taught. Standing lane: any round landing changes under `deploy/` outside the guide runs the sync before its wrap, or the wrap names it deferred in one line and the next session inherits it.

**Why:** The guide quotes concrete values for readability BY DESIGN (contract rule 1) — that readability is bought on credit, and the sync is the repayment schedule. A `Synced:` stamp is exactly as honest as the diff behind it, and the round-wrap lane puts the check where the drift is born: in the round that moved `deploy/`.

**Rejected:** *Hook/cron enforcement (a commit gate on deploy/)* — the icebox's standing precedent (R28's first resident): machinery without a demonstrated miss, while sessions run frequently and the wrap lane already asks. *Folding the guide into `/audit-guardrails`* — wrong lane twice: the audit sweeps standing LAW, proposals-only, at stage-boundary cadence; the guide is a living teaching document needing direct edits at deploy-round cadence. *Pure pointers (never quote a live value)* — would keep the guide trivially fresh and useless from zero; the guide round already chose readability plus stamps, this entry gives the stamps their engine. *Dates instead of hashes* — a date names when someone looked; the hash names exactly what they looked at.

**Revisit when:** the first two real syncs grade the shape (mapping too coarse → a page-owns-files table in the guide README; too heavy → narrow the re-read rule) · a sync is found missed across two consecutive deploy-touching rounds (lane failure — the icebox's hook entry is the same discipline's revival shape) · a second guide appears outside `deploy/` (the command generalizes or forks).

## R32 — The law travels: the template carries the portable working law

*(added 2026-08-17, template-law round, on the maintainer's instruction; method owned by the template's own `.claude/commands/law-sync.md` — `/law-sync` runs THERE, never here. The template: `~/Desktop/CurrPC/Programming/Example_Web_Server_Project`, its adoption entry R1. Sibling precedent: R31 — a derived layer kept honest by stamps and a diff.)*

**Decided:** The example template carries an ADAPTED copy of this project's portable working law: the covenant (R26), the decision guardrail in single-registry shape, the commit & push law (R9), the session cut points (R10), triage with the verdict stage (R22/R25 — `/triage`), the salvage rite (R27 — `/salvage`), the guardrail audit (R28 — `/audit-guardrails`), one-truth-one-home, and the registry / icebox / coverage / evidence-log record patterns as seeded starter files. Adaptation, not inclusion: parametrization is the template's own (its gates, its paths, one registry instead of two), and foreign ids are cited `WC R<n>` — pointers into THIS registry, never ids in its own, which starts fresh at its R1. Freshness is ON DEMAND and ONE-WAY: the template's `/law-sync` collects per-file `Synced: IA@<hash>` stamps, diffs this repo's law surfaces (CLAUDE.md, `.claude/commands/`, the record-pattern headers) per stamp, and re-adapts substance in place — this repo wins on substance, the template on parametrization; seeded record files are proposed-at, never overwritten; newly portable surfaces land, retirements are mirrored, stamps advance. NO standing duty lands on this repo's rounds: law moves freely here, the template catches up when the maintainer runs its sync, and the stamps make any gap mechanical.

**Why:** The template exists so the next project starts where this one arrived, and the working law is as transferable as the compose anchor — what one real multi-user deployment proved useful round after round will very probably be useful there (the maintainer's instruction, 2026-08-17). One-truth-one-home survives by R31's own resolution: the copy is a derived, stamped, subordinate layer — the copy that rots is the one WITHOUT a diff basis.

**Rejected:** *User-level global law (`~/.claude/`)* — one unparameterized text over every repo, unversioned, drifting invisibly. *Submodule/symlink sharing* — inclusion without adaptation; the law cites per-project surfaces (gates, registries, paths) that differ by design. *A standing wrap duty here, R31's lane mirrored* — wrong cadence: guide drift misleads a reader the same day, law drift costs nothing until the next project boots; on-demand matches the consumer. *A copy without stamps* — the disease R31 already named.

**Revisit when:** the first real `/law-sync` run grades the map (adaptation notes, sweep breadth) · a THIRD project adopts the pack (N pairwise syncs → weigh one dedicated law-template home) · the template's law deliberately forks (a fork is a ruling recorded on both sides, never drift).

---

## R33 — The research travels too: the template carries the evidence shelf

*(added 2026-08-17, shelf round, on the maintainer's instruction — "better to have too much and remove things, than to re-research everything, although we already did that once"; sibling and scope-extension of R32; the template's adoption entry is its R2. This re-judges the `/law-sync` not-ported list's 2026-08-17 "coverage CONTENT is World Console's territory" clause for research substance — re-judged by the philosopher directly, not by a sweep.)*

**Decided:** Beyond the working law (R32), the template carries two more layers. **(a) The remaining record patterns:** a design registry with the ruled-look law — no user-facing surface is built before its shape is written there and ruled (the G/F/P/A registry shape plus R14's spec-before-build precedent, single-file) — a build log (phases, checkboxed steps, verification lines; decisions stay in its one registry), and the design↔code audit rite (D-ids worst-first, Where/What/Decide lines, the verified-sound list, resolutions recorded dated). **(b) An inherited research shelf** (`research/` there): distilled reusable evidence from this project's corpus — where to launch (surfaces, staged rollout, monetization bounds), licensing & IP with the trademark method, Austrian founding & tax with the standing tripwires, the privacy & AI-Act paper kit, website & shop duties, hosting & names, and the laws of running an AI service (metering from turn one, provider-policy quicksand, custody lessons). Distillation, not wholesale: WC's stage specs, game-design content and registry CONTENT stay home; the shelf keeps dates, ⚠ re-verify contracts and primary sources, and marks WC's own choices as worked examples. Freshness: the shelf is a STAMPED SNAPSHOT outside `/law-sync`'s default sweep — refreshed only on the named scope (`/law-sync research`), which diffs the mother sections per stamp and re-distills; prunings there are preserved (a pruned section stays pruned). A project re-verifies a ⚠ fact before relying on it and lands the re-verified fact dated in its own log; the shelf entry is then superseded in place, never silently edited.

**Why:** The corpus cost weeks of receipts, and re-researching it per project is exactly the cost the template exists to kill — too-much-then-prune beats too-little-then-re-research (the maintainer's economics, stated 2026-08-17). Staleness is priced in, not wished away: every fast-moving fact already carries its ⚠ trigger from birth, and the stamps give any refresh a mechanical diff basis — R31/R32's own resolution, third verse.

**Rejected:** *Wholesale copies of 01/06/10* — 130 KB of WC-entangled threads would bury the reusable core a stranger project needs to find. *Pointer-only, no shelf* — a fresh repo should not need the mother checked out to learn that the reverse-charge VAT trap exists. *A standing research-sync duty on this repo's rounds* — the wrong cadence R32 already named: research moves here freely, the shelf catches up when asked.

**Revisit when:** the first real project consumes the shelf (grades the distillation and the prune marks) · a shelf fact bites as stale despite its ⚠ contract (tighten the contracts) · a third project adopts the packs (R32's own trigger — weigh the dedicated law+research template home then).

---

## R34 — The hub: one box, many doors

*(added 2026-08-18, the H0 ruling sitting — the five gate questions in [`research/hub/README.md`](../hub/README.md) answered by the maintainer, plus their same-sitting visibility ruling; design, workflow and build plan live in `research/hub/`, evidence in its `04-research-log.md` + the three 2026-08-18 ground-truth sweeps.)*

**Decided:** The stage-1 box and worldconsole.eu become the shared home of the maintainer's small projects: apex = hub page branded **Hausregel** (R16 — the H1 re-cut is the studio name's dated first public use; its evidence is archived per R16's €0 step), one subdomain family per tenant, one ingress Caddy (World Console's, promoted in place — extraction into its own compose project keeps its named trigger: a third tenant, or box ops chafing), tenants on a dedicated `ingress` network under the hardened anchor, per-tenant backups on the existing Storage Box. **World Console keeps priority — tenants are evictable guests.** Every tenant origin declares itself **public or private (login-gated), and the flip stays one fragment-edit cheap** (the maintainer's requirement, same sitting; shape in `research/hub/01-architecture.md` §Public and private tenants, contract in `02-wiring-workflow.md` §A.10). Tenant #2 is **Adsum** — repo canonical at `~/Desktop/CurrPC/Programming/adsum`, remoted private to github `SaibotiX/adsum` (verified 2026-08-18), its name register-clean in its own records (TMview EU/AT, classes 9/42) for the friends-stage exposure H2 needs. Touches, deliberately: R18's single-purpose rationale (widened, letter intact) · the 2026-08-17 exits picture (cancelling the basket now evicts every tenant — the dated note there lands at H1) · guide 30's rent-a-box default (second lane at H3, via `/guide-sync`). **R17 untouched** — the second-domain option was declined. Build: H1–H3 per `research/hub/03-build-plan.md`; the hub coverage row flips ● with this entry. *(revised 2026-08-19 by R35/R36: the hub becomes its own project — the promotion-in-place ingress line is overtaken (extraction at the split, by direction, ahead of its named trigger), the hub's records re-home to the WorldConsole repo at M0, and the page brands **World Console**, not Hausregel. The architecture itself — subdomain families, ingress network, tenant contract, visibility flip, tenant priority — stands unchanged.)*

**Rejected:** a box per project (~€11/mo each — the hub exists to retire exactly this) · a second neutral hub domain (~€6–9/yr; would have revised R17) · a PaaS layer (Coolify/Dokploy — real overhead, no added value at this scale) · path-based mounting (one origin, shared cookie/header blast radius) · wildcard TLS (custom Caddy build + DNS credentials for a convenience the explicit-names list doesn't need) — reasons with receipts in `research/hub/01-architecture.md` + `04-research-log.md`.

**Revisit when:** a third tenant approaches (the ingress-extraction trigger, plus the architecture's PaaS/generator revisit lines) · the hourly→12-month conversion executes (⚠ the netcup AGB re-verify rides that moment, per the hub log) · World Console's own needs ever chafe against a tenant (the priority rule applies; this entry is the license to evict).

---

## R35 — The hub is its own project: the split

*(added 2026-08-19, the split sitting — the maintainer raised the hub's independence before H1; the session's proposal ruled on point by point. Prior-art receipts: [`research/hub/04-research-log.md`](../hub/04-research-log.md) §2026-08-19.)*

**Decided:** The hub leaves the game's repo and becomes its own project, named **World Console** (the naming inversion is R36): home `~/Desktop/CurrPC/Programming/WorldConsole`, repo `git@github.com:SaibotiX/WorldConsole.git` (private), born from the Example template (the R32/R33 lanes travel with it). It owns what a landlord owns: the hub page, the ingress Caddy as its **own compose project** — R34's promotion-in-place line is overtaken; the extraction happens at the split, by the maintainer's direction, ahead of its named trigger — the tenant contract with onboarding/offboarding (`research/hub/` migrates there wholesale at M0), and the box-level runbook parts. The game stays tenant #1, `play.`/`vault.` untouched, its priority rule intact. **Domain: worldconsole.eu kept** (the maintainer's ruling) — R17 re-affirmed, and with R36 the domain matches the hub's own name. **Sequencing** — the maintainer delegated ("we do what is best for the project"); the session's pick: **M0 first** (the split + renames, [`research/hub/03-build-plan.md`](../hub/03-build-plan.md) §M0), then the **vision round** for the hub page in the new repo — the maintainer's stated design vision is explicitly too big for a side-build: the vision is spoken first, the session shapes it into the design spec, local renders make the seen-gate cheap and early — then H1–H3 re-cut there. Until M0 lands, this registry stays the one home; from M0 on the hub repo keeps its own records, citing back to R34–R36.

**Rejected:** building H1 here first and extracting later (a page built to be replaced against a stated vision, and ingress roots deepening right before the move) · the Adsum-early placeholder lane (nothing waits on a deadline; revivable the moment one appears).

**Revisit when:** a deadline appears for Adsum before the vision round closes (the placeholder lane revives) · M0 or the cutover surfaces anything the split's shape didn't foresee (back here for a dated note, never silently).

---

## R36 — The names recut: the game is GameMaster Bernd, the hub is World Console

*(added 2026-08-19, same sitting — the maintainer's ruling with its three follow-up answers taken in conversation (folder spelling · Hausregel's fate · the rename's reach); revises R16 and R34's branding line.)*

**Decided:** The maintainer inverts the names; all repos private. **The game** — this repo, "World Console" until today — becomes **GameMaster Bernd**: the full history migrates to `git@github.com:SaibotiX/GameMasterBernd.git`, the local folder renames `IA` → `GameMasterBernd` (no space, the maintainer's pick), and `SaibotiX/IA.git` stays intact as a **silent archive** — outside this entry's own text it is mentioned nowhere from M0 on. **The hub takes the name World Console** (home and repo in R35); its page **brands World Console — Hausregel retires** to the icebox with its clearance on file (revival condition there: a distinct studio/Einzelunternehmer name needed — founding (R21), or product names multiplying until an umbrella brand earns its keep). **The rename reaches the product** (the maintainer's pick): the game itself is titled GameMaster Bernd — M0 sweeps repo and records, and the live surfaces (landing, prompts, `config/`) land as one **seen-gated** step; in-fiction "world console" wording is judged case-by-case in the sweep, proposals to the maintainer (the term may survive where it reads as the in-world device rather than the title). Name consequences recorded, not deferred: the "World Console" clearance and first-use evidence (R16) stays on file and serves the name where it now stands — the hub — while **GameMaster Bernd starts with zero clearance work**: its register-index sweep (the Adsum-grade step) is due **before the game's card goes public on the hub page**; the professional search stays a filing-time step for whichever names file; the R16 €0 first-use archive re-cuts to the new names as their surfaces go live. Box-side operational names (`/home/deploy/world-console`, compose project `world-console`, network `world-console_ingress`) are re-cut at M0/the ingress cutover — the name belongs to the hub going forward. Nothing implemented at this sitting — the maintainer's instruction was record-only; M0 owns the execution.

**Rejected:** the folder spelled with the space ("GameMaster Bernd" — a quoting tax on every path forever) · keeping Hausregel on the page front or as a parallel studio brand (two names where one now does the work; the icebox holds the revival) · a repo-only rename with the product keeping "World Console" (a title collision with the hub the moment its card renders).

**Revisit when:** the GameMaster Bernd register-index sweep runs (M0 or the vision round — before the public card, binding) · Hausregel's icebox condition fires · any conflict surfaces on the new game name (R16's Praey lesson: pre-launch, renaming is cheap).
