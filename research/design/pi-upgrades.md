...
| 2 | widget `render(width)` width contract: pi-tui's main screen THROWS on any rendered line wider than the terminal — overlays it truncates itself, widgets it kills the app over (the 59-column strike-banner crash, 2026-08-17). Every widget render ends in `fitLines` (a last-step cap); composition may aim wider (margins, `Math.max` floors, art), the cap holds the contract | `extension/index.ts` widgets | tty-probe narrow leg (`WC_NARROW=1`: a standing world-strike at 59 columns) |
...
| 2 | the WC-15 nudge (2026-08-17) rides `before_agent_start`'s RESULT `message` field (`BeforeAgentStartEventResult.message`) — pi injects it into the turn's context as a user-role custom message beside the seeker's turn; documented type, but if pi stops honoring the field the nudge vanishes SILENTLY (the standing-layer offers keep working, the teeth don't) | `extension/index.ts` §sweep | changelog watch; no automated guard — the next batch's WC-15 count is the tell |
