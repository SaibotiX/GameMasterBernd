...
- **The ledger lives in the session** — every game event (`world`, `player_named`, `mood_set`, `websearch_ban`, `redemption`, `search_requested/performed/refused/failed`, `truth`/`truth_retracted`, `chronicle`, `place`/`place_chronicled`, `persona`, `quest`, `item`) is a custom entry (`world-console.ledger`) in pi's session file. All state is derived by folding the **current branch** (`sessionManager.getBranch()`), so:
