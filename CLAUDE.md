...
GameMaster Bernd: a terminal story game built as extensions on the pi coding agent. The main `README.md` explains how the engine works; this file is about how to work ON the project without breaking its records. **The renames executed at M0 (2026-08-20; ruled 2026-08-19, R35/R36):** the game — "World Console" until M0 — is **GameMaster Bernd** (repo `SaibotiX/GameMasterBernd`, folder `~/Desktop/CurrPC/Programming/GameMasterBernd`); "World Console" passed to the hub project, now its own repo. Box-side operational names re-cut at H1a step 2 (2026-08-21 — R36's clone-path pass) and **LIVE since the cutover sitting the same day (H1a step 3):** the box runs this repo from `/home/deploy/gamemaster-bernd` as tenant #1 behind the World Console hub's ingress — the hub repo's runbook owns the box's front door, apex, firewall and landlord lanes since then (`/home/deploy/world-console` is ITS clone now). Deliberately KEPT: `WC_`/`wc-` prefixes, the on-disk `world-console.*` session types, the image tag `world-console:latest`, the borg repo `borg/worldconsole` (engine/data surface and live archives — reasons in the runbook's step-2 state note and backup.sh's header). Dated records keep their historical wording.
...
| `research/hub/` | a pointer stub only — the hub's design home migrated wholesale at M0 (2026-08-20, R35/R36) to the **World Console** repo (`~/Desktop/CurrPC/Programming/WorldConsole`, its `hub/` + `records/`); the hub's questions are answered THERE — and since the H1a cutover (2026-08-21) its runbook owns the box's landlord truth (front door, apex, firewall, box backup + pager); this repo's runbook keeps the tenant lanes |
...
| `deploy/` | the stage-1 hosting machinery. `deploy/README.md` is the runbook — live-ops truth for the game's TENANT lanes since the 2026-08-21 cutover (seats, store, its own borg + pager; the box's front door is the hub repo's), procedures, dated state notes; `deploy/guide/` is the teaching layer — from-zero explanations, how-tos, reference and the next-project blueprint, each page stamped with the deploy/ commit it was synced against. Where they disagree, the runbook wins. A round that lands changes under `deploy/` outside the guide runs `/guide-sync` (R31) before its wrap |
