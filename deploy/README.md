...
*(State 2026-08-10, backups round: **the lane is live and the invite's gate fell** — borg 1.4.0 on the box, the repo encrypted repokey-BLAKE2b on the Storage Box (BX11 Falkenstein; host keys pinned in root's known_hosts, all three verified against Hetzner's published fingerprints). The maintainer's at-order paste turned out to be the box's own `storagebox_ed25519` pubkey, so the lane opened keys-only from the first minute — the box password never left the manager, exactly as 06 §2026-08-09 ordered. First archive `nightly-2026-08-09T22-27-52Z`: five volumes staged read-only + the store + all six box-local state paths — 60 files, 4.56 MB → 3.84 MB encrypted — prune + compact clean, the timer armed for 05:11 nightly (Persistent). **The restore gate ran green the same night against the real repo:** data-tobias's tar extracted from the archive, rebuilt into a scratch volume, a scratch container booted against it at the game's own data path, and the chronicle `world/dragon-realm/…/ledger.md` read whole — sha256 identical to the live volume (`bc49d259…`). Passphrase custody: `/root/worldconsole-borg.pass` + the key export `/root/worldconsole-borg.key`, both root-600; the maintainer's one errand: read both into the password manager (§Backups carries the command). Two landmines en route, each its own commit: the minted script landed without its exec bit (`227edb7`), and the staging/restore containers needed `--user 0` — the player uid can neither write the root-700 staging dir nor read caddy's root-owned volume files (`518c0bb`). One stray found riding along: `world-console_sessions-drain-test`, the house-lane round's probe volume, still exists and enters every archive — flagged for the maintainer's call, not deleted. **Ruled 2026-08-10** (the call delegated to the session): deleted — the volume was EMPTY (the drain test's turns were bare API knocks; no sitting ever wrote a session), keyless and storeless; its 171-micro ledger rows stay under the honest handle as billing truth, and the removed-player reconcile rule reads them benignly.)*

*(State 2026-08-10, holiday watch: **the box is set to run the week alone** — swept the evening the maintainer left, with jakob's and kilian's first sittings ahead. Found green: all four seats pinned to the day's image under `restart: unless-stopped`, every timer armed and enabled (reaper 5-min · sweep 04:19 · reconcile 04:47 · backup 05:11, Persistent), the firewall service standing, doors 200/404/404 from the box under Let's Encrypt good to Nov 6, disk at 4%, no swap, zero failed units, the morning's archive landed and tonight's stages all eight friend volumes (jakob/kilian/testuser minted after the 05:14 run — tonight is their first night aboard). Ledger: four seats × the $10 placeholder, pennies spent — the worst-case lane week is $40, under the $55 kill-switch by construction. The silent-failure gap closed as §the pager (both halves proven on the real topic that afternoon — the two test pings on the maintainer's phone, red `wc-alert-proof` and green `pre-holiday-proof`, are the receipt). Residues named, not fixed: no watcher lives off the box, so a dead box is a silent breakfast (the heartbeat's absence is the signal; friends saying "it's down" the second); unattended-upgrades stays absent by choice for the week — no surprise reboots, patch by hand after; a friend who drains the grant waits walled until Sept 1 unless the maintainer asks for a raise (one keys.json edit — the gateway re-reads per request). The week needs nothing from the dev machine.)*

*(State 2026-08-20, address swap: **the Impressum carries the rented address** — R19's pick executed (revised 2026-08-20; terms in roadmap 06 §2026-08-20): `WC_IMPRESSUM_ADDRESS` in the box `.env` now holds `c/o RA Matutis, Fürstenstraße 5, 5400 Hallein` — the address `<dd>` under the unchanged name line — caddy force-recreated, all three pages 200 and the rendered block verified from the box; **the private street is off the page**. The Wohnort-municipality line was ruled OMITTED (maintainer, 2026-08-20; the medium-confidence MedienG floor reading stays on file as accepted residual risk). The firm's standing duties — inform-mail per publication, no confidential post to the firm, no Google-Business use, renewal 2027-08 — are tracked maintainer-side (their instruction, same day; sessions released from chasing them). The 2026-08-09 first-use set stays untouched box-local as dated evidence. **Closed 2026-08-20: the eyeball green** — "the Impressum is perfect", the maintainer's own words; the swap round closes whole.)*
...
| Backups + restore test (item 13) | done (2026-08-10) — the borg lane nightly at 05:11 to the BX11 (repokey-BLAKE2b, prune ≤ 28 days binding §deletion), the restore gate green against the real repo the first night, R18's pull leg proven to the maintainer's machine (§Backups + the backups-round state note); the invite's gate has fallen |
| **The first invite** (friends #1–2) | done (2026-08-10) — jakob + kilian: part one answered on discord the same day (both yeses, both 18+; rows in `consents.md` against note 2026-08-09), minted on the player image and proven from outside (401 bare / 200 authed / 404 for a stranger's token); part two out of band is the maintainer's half. The same scoped `up -d` recreated `wc-tobias` onto the player image — the R30 rider closes |
| The pager (holiday watch) | done (2026-08-10) — store-sweep/reconcile/backup failures ring the ntfy topic, the backup's success is the ~05:14 heartbeat; both halves proven on the real topic (§the pager) |

## Next round (updated 2026-08-10): the invite is served — what stands open

The first invite landed on the maintainer's yes-in-hand (state row above): jakob and kilian are minted, live behind their doors, born on the player image with the R12 placeholder grant. What remains is the maintainer's half and two named threads:

- **Part two, out of band, once** (§per-friend onboarding step 4) — the doors and pairs were printed at mint, into the maintainer's session alone.
- **Both seats opened on the default world** (`dragon-realm`; the mint writes no world env — the commented `WORLD_CONSOLE_WORLD` line is the hand affordance). Flipping a seat to `star-frontier` before its first sitting is lossless: uncomment the line in that friend's override block, `docker compose up -d wc-<name>`. Once a first tale stamps, the stamp law keeps it whole either way — and since `/worlds` (below) a friend binds their own world for every tale after the first; only the first tale opens on the seed.
- **`wc-testuser`: ruled 2026-08-10 — a seat like any other.** The standing flag closed on the maintainer's word, raised when the seat — deliberately held out of the `/worlds` rollout while its fate was open — still showed the morning image (a container pins its image at creation; only a recreate moves it). It stays, and it rolls with every update like any friend's seat; rolled onto the `/worlds` image the same day.
- **The `/worlds` picker** — ruled and built the same day ("implement `/worlds` as command sixteen"; R30 carries the dated revision, build log Phase 3 the receipts). Image `99dc64f` verified on the box — its in-container probe now TYPES `/worlds` and expects the listing — and all three seats rolled onto it client-aware (each seat's healthz answered no live player; a seat with someone at the console would have held). A friend binds their own world for the next `/new`; the mint's env line stays the operator seed, and the seat's choice file rides the data volume (and so the nightly borg archive).

*(2026-08-10, player-UI round — R30 jumped this queue on the maintainer's ordering: the image carries `WC_PLAYER_UI=1` + `PI_OFFLINE` and the entrypoint seeds `quietStartup`, so a friend's first screen is the banner, the game footer line and the fifteen commands, not the workshop. The box rebuilt before the invite as ordered; verify leg 2 asserts the player console in-container.)*

The backups round's errands closed 2026-08-10: passphrase + key read into the password manager (maintainer); the stray drain-test volume ruled and deleted (state note above); borg on the dev machine, localcheck's leg green there (§Backups). En route, the session dry-ran the first fully-timered night and disarmed two false pages before 04:48 ever fired: reconcile gained its epoch floor and the removed-player rule (the script header owns the semantics; localcheck proves all four verdicts; commit `1c7ae9f` tells the story). The 24× "mismatch" the dry-run surfaced was NO pricing bug — the 2026-08-09 morning sittings were the maintainer's own pre-lane play at pi's default Opus, correctly priced, real spend on their own account (~$2.97) that the lane's ledger rightly never metered.

The week itself belongs to the holiday watch (state note above): the box runs alone, the pings speak. The round on return opens with `pull-sessions.sh` and the analysis kit over the friends' first sittings — the first real user data.
