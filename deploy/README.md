...
Riders into the round: the wake-cycle eyeball is still owed (§shipper-round state note's seen-before-done) · 2026-08-31 ends Sonnet's intro pricing — binds only if Sonnet enters the mix; the keeper stays the tuned Haiku by every receipt, and the model mix is design + telemetry's question, outside gate zero's ruling.
