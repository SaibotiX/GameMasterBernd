...
0. **Gate zero — the house-key ruling lands as its own R entry** (06 §2026-08-08 the house key recommends **Anthropic direct, prepaid**; the maintainer confirms the early-minted org key is exactly that, or rules otherwise), executing the at-account-creation re-verify trigger (prices, rate-limit tier, retention pages, spend-cap mechanics) and confirming the workspace spend cap is SET (item 8's standing note). One records commit; the coverage row's open edge closes.
