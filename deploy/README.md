...
| Disclosure + intro (items 9 + 11) | done (2026-08-09) — the words round: landing ground live at the root (address env is the one errand), first-run notice + 50(2) marking in the page, intro ready with the LICENSE line, consent gate live, §deletion dry-run green on production, side-call tag deployed (§words-round state note); site + notice eyeballs ride to the maintainer; vault (items 6–7) on ice with R11 |
| Backups + restore test (item 13) | **the next round — gates the first invite** (ruled 2026-08-09): Hetzner Storage Box purchase (maintainer errand) + borg (retention ≤ 28 days per §deletion) + one proven restore |

## Next round (picked 2026-08-09): the house lane — 02 item 8, R12
