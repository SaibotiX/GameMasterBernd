...
| Shipper + item 12's host half (items 10 + 12, R13) | done (2026-08-09) — mirror/seal/sweep live at every seam, store verified + compacted daily, reaper + waker + disk watch armed; tobias's back-catalog shipped on first boot; pull and deletion proven end to end (§shipper-round state note). Two rulings open: encrypted-at-rest (§the store ⚠), quota enforcement (§the reaper) |
| House lane (item 8, R12) · disclosure + intro (items 9 + 11) | own rounds, the next to pick — the onboarding gate above names what must be live before friend #1; vault (items 6–7) on ice with R11 |
